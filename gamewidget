...
void GameWidget::createLabel()
{
    while(true){
        ...
        //背景方框
        MyLabel *label=new MyLabel(Create_num[index]);
        int x = col * colWidth + xos;
        int y = row * rowHeight +yos;
        ...
        for(int i = 0;i< arr.size(); i++)
        {
            ...
            for(int j = 0; j< arr1.size(); j++)
            {
                ...
                if(oldValue != 0)
                {
                    ...
                    int x = j * colWidth + xos;
                    int y = i * rowHeight +yos;
                    label->setGeometry(x,y,66,66);
                    label->setParent(m_bgWidget);
